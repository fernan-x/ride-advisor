import { IForecastRepository } from "../domain/forecast.repository";
import { DailyForecast, WeatherData } from "../domain/forecast.domain";
...
export class OpenMeteoForecastRepository implements IForecastRepository {
  ...
  async getForecast(city: string, days: number = 7): Promise<DailyForecast[]> {
    const coords = CITY_COORDINATES[city] || CITY_COORDINATES[DEFAULT_CITY];
